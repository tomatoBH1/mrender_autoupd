...
if not isSampLoaded() or not isSampfuncsLoaded() then return end
    ...
    sampAddChatMessage("[MRender - PRO] {D5DEDD}Данная версия является Pro Version! Версия: {01A0E9} 1.7", 0xFF0000)
	...
	sampAddChatMessage('[ФПС] {D5DEDD}При использовании игра теряет около 5-9 fps', 0xFF0000)
		...
		for id = 0, 2048 do
            ...
            if result then
				...
				if rlen.v and text:find("Лён") then
                    ...
                    local resX, resY = getScreenResolution()
                    if wposX < resX and wposY < resY and isPointOnScreen (posX,posY,posZ,1) then
                        renderFontDrawText(font,' Лен', wposX, wposY,test2)
                    end
                end
				if rlen.v and text:find("Лён") then 
                    if isPointOnScreen (posX,posY,posZ,1) then
				...
				if rhlopok.v and text:find("Хлопок") then
                    ...
                    local resX, resY = getScreenResolution()
                    if wposX < resX and wposY < resY and isPointOnScreen (posX,posY,posZ,1) then
                        renderFontDrawText(font,' Хлопок', wposX, wposY,test2)
                    end
                end
				if rhlopok.v and text:find("Хлопок") then 
                    if isPointOnScreen (posX,posY,posZ,1) then
				...
				if rderevo.v and text:find("Нажмите ALT, чтобы собрать плоды") then
                    ...
                    local resX, resY = getScreenResolution()
                    if wposX < resX and wposY < resY and isPointOnScreen (posX,posY,posZ,1) then
                        renderFontDrawText(font,' Дерево', wposX, wposY,test2)
                    end
                end
				if rderevo.v and text:find("Нажмите ALT, чтобы собрать плоды") then 
                    if isPointOnScreen (posX,posY,posZ,1) then
				...
				if rgrove.v and text:find("Grove Street") then
                    ...
                    local resX, resY = getScreenResolution()
                    if wposX < resX and wposY < resY and isPointOnScreen (posX,posY,posZ,1) then
                        renderFontDrawText(font,' Граффити Grove Street', wposX, wposY,test2)
                    end
                end
				if rgrove.v and text:find("Grove Street") then 
                    if isPointOnScreen (posX,posY,posZ,1) then
				...
				if rballas.v and text:find("East Side Ballas") then
                    ...
                    local resX, resY = getScreenResolution()
                    if wposX < resX and wposY < resY and isPointOnScreen (posX,posY,posZ,1) then
                        renderFontDrawText(font,' Граффити Ballas', wposX, wposY,test2)
						renderDrawLine(x10, y10, wposX, wposY, test1, test)
                    end
                end
				if rballas.v and text:find("East Side Ballas") then 
			    ...
			    if rrifa.v and text:find("The Rifa") then
                    ...
                    local resX, resY = getScreenResolution()
                    if wposX < resX and wposY < resY and isPointOnScreen (posX,posY,posZ,1) then
                        renderFontDrawText(font,' Граффити Rifa', wposX, wposY,test2)
						renderDrawLine(x10, y10, wposX, wposY, test1, test)
                    end
                end
				if rrifa.v and text:find("The Rifa") then 
				 ...
				 if raztec.v and text:find("Varrios Los Aztecas") then
                    ...
                    local resX, resY = getScreenResolution()
                    if wposX < resX and wposY < resY and isPointOnScreen (posX,posY,posZ,1) then
                        renderFontDrawText(font,' Граффити Aztecas', wposX, wposY,test2)
						renderDrawLine(x10, y10, wposX, wposY, test1, test)
                    end
                end
				if raztec.v and text:find("Varrios Los Aztecas") then 
				...
				if rnw.v and text:find("Night Wolves") then
                    ...
                    local resX, resY = getScreenResolution()
                    if wposX < resX and wposY < resY and isPointOnScreen (posX,posY,posZ,1) then
                        renderFontDrawText(font, 'Граффити Night Wolves', wposX, wposY,test2)
						renderDrawLine(x10, y10, wposX, wposY, test1, test)
                    end
                end
				if rnw.v and text:find("Night Wolves") then 
				...
				if rvagos.v and text:find("Los Santos Vagos") then
                    ...
                    local resX, resY = getScreenResolution()
                    if wposX < resX and wposY < resY and isPointOnScreen (posX,posY,posZ,1) then
                        renderFontDrawText(font,' Граффити Vagos', wposX, wposY,test2)
						renderDrawLine(x10, y10, wposX, wposY, test1, test)
                    end
                end
				if rvagos.v and text:find("Los Santos Vagos") then 
...
function imgui.OnDrawFrame()
	...
	imgui.SetNextWindowSize(imgui.ImVec2(535, 330), imgui.Cond.FirstUseEver)
	imgui.Begin(u8'MRender v1.7 Pro Version', main_window_state, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize)
	imgui.BeginChild('##menu', imgui.ImVec2(150, 300), true)
	imgui.CenterText(u8'Меню')
	if imgui.Button(u8'Рендер', imgui.ImVec2(135, 59)) then selected = 1 end
	imgui.Separator()
	if imgui.Button(u8'Рендер граффити', imgui.ImVec2(135, 59)) then selected = 2 end
	imgui.Separator()
	if imgui.Button(u8'Об авторе', imgui.ImVec2(135, 59)) then selected = 3 end
	imgui.Separator()
    if imgui.Button(u8'Кастомизация', imgui.ImVec2(135, 59)) then selected = 4 end
	imgui.EndChild()
	imgui.SameLine()
	if selected == 1 then
		imgui.BeginChild('##render', imgui.ImVec2(367, 300), true)
		...
		imgui.Checkbox(u8"Деревья", rderevo)
		imgui.Separator()
		imgui.CenterText(u8'Ивенты(Пасха 2023)')
		imgui.Separator()
		save1()
		imgui.EndChild()
	end
	if selected == 2 then
		imgui.BeginChild('##getto', imgui.ImVec2(367, 300), true)
	...
	if selected == 3 then
		imgui.BeginChild('##information', imgui.ImVec2(367, 300), true)
		imgui.CenterText(u8'Информация')
		imgui.Separator()
		imgui.Text(u8'Автор: Tomato')
		imgui.Text(u8'Обновление от 15.04.2023')
		imgui.Text(u8'Запустить скрипт: /mrender')
        imgui.Text(u8'Раздел заполнен не до конца')
		imgui.EndChild()
	end
    if selected == 4 then
		imgui.BeginChild('##settings', imgui.ImVec2(367, 300), true)
